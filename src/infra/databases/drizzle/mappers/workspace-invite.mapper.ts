import { UserRole } from '@constants/enums';
import * as schema from '@infra/databases/drizzle/schema';
import { WorkspaceInvite } from '@modules/workspace/entities/WorkspaceInvite';

type InviteSelect = typeof schema.workspaceInvites.$inferSelect;
type InviteInsert = typeof schema.workspaceInvites.$inferInsert;

export class WorkspaceInviteMapper {
  static toDomain(raw: InviteSelect): WorkspaceInvite {
    const result = WorkspaceInvite.create(
      {
        code: raw.code,
        workspaceId: raw.workspaceId,
        role: raw.role as UserRole,
        createdBy: raw.createdBy,
        expiresAt: raw.expiresAt,
        usedAt: raw.usedAt,
        usedBy: raw.usedBy,
      },
      raw.id,
    );

    if (result.isLeft()) throw result.value;
    return result.value;
  }

  static toDatabase(entity: WorkspaceInvite): InviteInsert {
    return {
      id: entity.id,
      code: entity.code,
      workspaceId: entity.workspaceId,
      role: entity.role,
      createdBy: entity.createdBy,
      expiresAt: entity.expiresAt,
      usedAt: entity.usedAt ?? null,
      usedBy: entity.usedBy ?? null,
    };
  }
}
